...
因为 $Z_i$ 是不可观测的，因此也不可控制。FE估计利用time-demeaning (within transformation) 来消除 $Z_i$:
$$\begin{align*}
...
---
# 固定效应回归标准误
固定效应回归模型的一大假设为$$(X_{i1}, X_{i2}, \dots, X_{i3}, u_{i1}, \dots, u_{iT}), i = 1, 2, \cdots, n$$ 是一个联合分布的iid样本。

这就要求观测值是uncorrelated across entities。 但是我们允许 X autocorrelated within entities
...
- miles: 州际平均驾驶里程
- drinkage: 州际法定最小饮酒年龄
- dringagc: drinkage的离散化变量，将年龄分为四个区间[18,19), [19,20), [20,21) 和 [21,22].。
